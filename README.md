...
| 缩小尺寸 | 8 种预设（1080p / 720p / 540p / 360p / 25% / 50% / 75% / 自定义） |
| 同时处理 | 先截取再缩小，一次搞定 |
| 批量处理 | 多选视频 → 一键处理 → 下载 **单个 ZIP 压缩包** |
| 本地处理 | 视频不上传服务器，浏览器内完成 |
| 输出格式 | 单文件：WebM；批量：包含所有 WebM 的 ZIP |

## 已知限制

- 老版本 iOS Safari 对 `MediaRecorder` 支持有限，建议用 Chrome / Edge 或较新 iOS
- 处理时间约等于视频时长（因为用 Canvas + MediaRecorder 实时录制）
- 输出的 ZIP 无压缩（视频本身已压缩，再压意义不大）
